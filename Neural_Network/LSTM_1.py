...
使用正弦函数和余弦函数来构造时间序列，
正余弦函数之间是成导数关系，
可以构造模型来学习正弦函数与余弦函数之间的映射关系
通过输入正弦函数的值来预测对应的余弦函数的值

弦函数值和余弦函数值不是一一对应关系
对于传统的神经网络来说，它仅仅基于当前的输入来预测输出，对于这种同一个输入可能对应多个输出的情况不再适用。

取正弦函数的值作为LSTM的输入，来预测余弦函数的值。
基于Pytorch来构建LSTM模型，采用1个输入神经元，1个输出神经元，16个隐藏神经元作为LSTM网络的构成参数
平均绝对误差（LMSE）作为损失误差 MSELoss()
使用Adam优化算法来训练LSTM神经网络 torch.optim.Adam(lstm_model.parameters(), lr=1e-2)
'''
...
class LstmRNN(nn.Module):
    ...
    def forward(self, _x):
        ...
        x = x.view(s * b, h)  # view()的作用相当于numpy中的reshape，重新定义矩阵的形状。
...
if __name__ == '__main__':
    # create database
    data_len = 200
    t = np.linspace(0, 12 * np.pi, data_len)  # linspace返回间隔均匀分布的数值序列
    sin_t = np.sin(t)
    cos_t = np.cos(t)
    ...
    optimizer = torch.optim.Adam(lstm_model.parameters(), lr=1e-2)  # 优化器
    ...
    # print(train_y_tensor[1, 1])
    # print(predictive_y_for_training[[1, 1]])
